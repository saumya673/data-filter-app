...
export function LastReviewFilterModal() {
  const open = useLastReviewFilterStore((s) => s.open);
  const close = useLastReviewFilterStore((s) => s.closeModal);

  const draft = useLastReviewFilterStore((s) => s.draft);
  const setDraft = useLastReviewFilterStore((s) => s.setDraft);

  const apply = useLastReviewFilterStore((s) => s.apply);
  const clear = useLastReviewFilterStore((s) => s.clear);

  return (
    <LocalizationProvider dateAdapter={AdapterDayjs}>
      <Dialog open={open} onClose={close} fullWidth maxWidth="sm">
        <DialogTitle>Last Review</DialogTitle>
